...
class TransformerEncoderMLPRegressor(nn.Module):
    """
    Feature tokens -> Transformer Encoder (MLP FFN) -> mean pooling -> MLP head -> linear(output_dim)
    """
    ...
    def __init__(
        ...
        super().__init__()
        del kan_grid, kan_k, kan_seed, ff_chunk_size, head_chunk_size  # kept for config parity
        ...
        self.mlp_head = MLPHead(d_model=d_model, hidden_dim=mlp_hidden_dim, dropout=dropout)
        self.reg_head = nn.Linear(mlp_hidden_dim, output_dim)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train/Evaluate dataset-agnostic Transformer Encoder - MLP tabular regressor."
    )
    ...
    parser.add_argument(
        "--checkpoint_path",
        type=str,
        default="best_model_mlp.pt",
        help="Path to save/load the best validation checkpoint.",
    )
    parser.add_argument(
        "--lr_plot_path",
        type=str,
        default="transformer-encoder-mlp-tabular-lr.png",
    )
...
def main() -> None:
    ...
    model = TransformerEncoderMLPRegressor(
        ...
        mlp_hidden_dim=args.kan_hidden_dim,
    ...
    print("\n=================================================")
    print("MODEL: Transformer Encoder - MLP (Tabular Regression)")
    print("=================================================")
