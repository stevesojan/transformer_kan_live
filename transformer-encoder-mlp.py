...
class KANHead(nn.Module):
    ...
    Kept identical to transformer-encoder-kan.py.
    """
...
class TransformerEncoderMLPRegressor(nn.Module):
    """
    Feature tokens -> Transformer Encoder (standard MLP FFN) -> mean pooling
    -> KAN head -> linear(2)
    """
    ...
    def __init__(
        ...
        super().__init__()
        del ff_chunk_size  # kept in signature for config parity.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Train/Evaluate Transformer Encoder - MLP regressor on UCI Energy Efficiency."
    )
    ...
    # Keep all configs aligned with transformer-encoder-kan.py.
